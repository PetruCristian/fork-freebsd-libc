...
def extract_libc(src_root, output_dir, arches=["amd64", "aarch64"]):
    ...
    # Collect all relevant files in directories we used to maintain structure
    print("\nCollecting auxiliary files (Makefiles, sources, headers, etc.)...")
    used_dirs = set(os.path.dirname(f) for f in all_files)
    for d in list(used_dirs):
        if not d.startswith(src_root): continue
        for f in os.listdir(d):
            # Include Makefiles, headers, and all source files to ensure generic implementations are present
            if f.startswith("Makefile") or f.endswith((".inc", ".map", ".def", ".mk", ".c", ".h", ".S", ".s")):
